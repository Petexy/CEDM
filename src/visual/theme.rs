...
/// Whether the displayed palette has reached `name` exactly.
///
/// The desktop manager uses this at the session boundary: LineXinBar starts
/// from its saved authored palette, so the greeter's last wallpaper frame must
/// finish the same palette transition before it releases the display.
pub fn settled_to(name: &str) -> bool {
    let Some(index) = accent_index(name) else {
        return false;
    };
    let transition = lock_transition();
    transition.target == index
        && transition.progress >= 1.0
        && transition.shown == ACCENTS[index].theme.rendered()
}
...
mod tests {
    ...
    /// The sparkles are part of the look a person starts with: a screen that
    /// has been told nothing about an account draws the current with them.
    #[test]
    ...
    #[test]
    fn settlement_reports_only_the_exact_rendered_endpoint() {
        with_accent("Purple", || {
            assert!(settled_to("Purple"));
            assert!(!settled_to("Blue"));
            assert!(preview_accent("Blue"));
            assert!(!settled_to("Blue"));
            assert!(!animate(ACCENT_CHANGE));
            assert!(settled_to("Blue"));
            assert!(!settled_to("not-an-accent"));
        });
    }

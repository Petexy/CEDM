...
struct Globals {
    ...
    // x is the wallpaper and y is every mark drawn on it; they are separate
    // settings and either may be either way round. z is the shape of a user's
    // own picture, which this program never draws. w is one where the sparkles
    // the current carries are turned on, under the shell's Theme > Particles,
    // and nought where they are not.
    style: vec4<f32>,
};
...
// The wallpaper a *session's* user chose, where they chose one. This login
// screen never draws one and the binding is deliberately a single transparent
// texel: the picture is a file under one account's home directory, and a greeter
// standing in front of every account on the machine is not in a position to open
// it — see `accent::CUSTOM_WALLPAPER`, where that setting is read and answered
// with the shell's own scene.
//
// It is here so that `wallpaper()` can stay the shell's function verbatim, which
// is the whole contract this file is under. The branch it feeds is never taken.
...
// in the middle of the interval it is given. Kept in step with LineXinBar's
// `BAND_EDGE_SAMPLES`, which is where it was measured.
...
// ---------------------------------------------------------------------------
// Background: the wallpaper, once for each display.
// ---------------------------------------------------------------------------
//
// One instance per display, each covering that display's own rectangle of the
// surface, because the wallpaper is a function of where you are on a *screen*.
// Drawn once across a surface that spans a row of monitors it would be one
// picture stretched over two panels of different shapes — and the session's
// compositor, which gives every output its own surface, would replace it at
// sign-in with two pictures. The seam that appears there is this pass.

struct BackgroundOut {
    @builtin(position) clip: vec4<f32>,
    // Where this pixel is on its own display, 0..1 in each direction.
    @location(0) uv: vec2<f32>,
    // And which display that is: x, y, width, height in surface pixels.
...
fn vs_background(
    ...
    // Two triangles over exactly one display. Not the usual oversized single
    // triangle: that one is trimmed by the viewport, and here what is beside
    // the display being drawn is the next display along rather than nothing.
    ...
    out.clip = to_clip(display.xy + corner * display.zw);
    out.uv = corner;
    out.display = display;
    return out;
}
...
fn fs_background(in: BackgroundOut) -> @location(0) vec4<f32> {
    ...
    // On the surface, for the rectangles below, which are placed on it; the
    // wallpaper itself is asked about `uv`, which is where this pixel is on
    // the display it is actually on.
    let px = in.display.xy + in.uv * in.display.zw;
    ...
    var footprint = 1.0 / max(in.display.zw, vec2<f32>(1.0));
    ...
    for (var i = 0u; i < covers; i = i + 1u) {
        ...
        if (inside_bounds) {
            ...
            if (corner > 0.0) {
                coverage = corner;
                uv = in.uv;
                footprint = 1.0 / max(in.display.zw, vec2<f32>(1.0));
    ...
    // The display's own shape, not the surface's. A wallpaper drawn to the
    // shape of two monitors side by side is drawn to the shape of neither.
    let aspect = in.display.z / max(in.display.w, 1.0);
...
struct QuadIn {
    ...
    // The display this pane is standing on: x, y, width, height in surface
    // pixels. What the pane transmits where nothing was drawn behind it is the
    // wallpaper of *this* display, evaluated exactly as that display's own
    // background pass evaluated it. All zeros is the whole surface.
    @location(9) display: vec4<f32>,
};
...
//
// `display` is the screen the pane is standing on, as its rectangle on the
// surface. The two things behind it are read in different coordinates and that
// is not an inconsistency: the snapshot is a picture of the whole surface and
// is sampled where the pane is on it, while the wallpaper is a function of a
// *display* and has to be asked at the same place its own pass asked — or a
// pane on the second monitor of a row transmits the first monitor's wallpaper,
// which is a window onto the wrong screen.
fn behind_at(px: vec2<f32>, lod: f32, soften: f32, display: vec4<f32>) -> vec3<f32> {
    ...
    // A pane that was never told which display it is on is on the only one
    // there is, which is the whole surface.
    var screen = display;
    if (screen.z <= 0.0 || screen.w <= 0.0) {
        screen = vec4<f32>(0.0, 0.0, globals.resolution);
    }
    let local = clamp((px - screen.xy) / screen.zw, vec2<f32>(0.0), vec2<f32>(1.0));
    let aspect = screen.z / max(screen.w, 1.0);
    ...
    let below = wallpaper(
        ...
        1.0 / max(screen.zw, vec2<f32>(1.0)),
    );
...
// --- the material a measured shape is drawn in -----------------------------
//
// `lxb-desktop`'s `glyph_material` and the three constants it reads, carried
// across whole. This greeter draws one thing in it — the letters of the clock,
// which are cut out of the bundled face and measured into distance fields by
// `visual::letters` — and it must be the shell's own material and not a second
// one that resembles it: the login screen hands over to the shell four seconds
// later, and two readings of water in front of the same person is exactly the
// seam this project exists to avoid. If it changes there it has to change here.
//
// Half the range a glyph's distance field spans, as a fraction of its atlas
// cell. `visual::letters::SDF_RANGE` encodes it and this undoes it; the two are
// one number and have to move together.
const GLYPH_SDF_RANGE: f32 = 0.125;

// Edge of one atlas cell in texels, which is `visual::CELL` — twice the
// shell's, because this atlas also holds photographs of people. The shader
// needs it to know what a slope of one looks like in the field it is reading.
const GLYPH_CELL: f32 = 256.0;
...
fn glyph_material(in: QuadOut) -> vec4<f32> {
    ...
    //
    // Three texels here where the shell reads one and a half, and that is the
    // shell's number rather than a departure from it: an arm is a filter width
    // and belongs to the *cell*, and this atlas's cells are twice the shell's.
    // Read at a texel and a half of a 256-pixel cell it comes to half the filter
    // — and on a clock drawn at nearly the size of its own cell, that leaves the
    // eight-bit field's own steps in the surface normal, which a specular of the
    // forty-second power lays along every straight stem as a row of dashes. Found
    // on screen, on the hour, and nowhere else.
    let arm = texel * 3.0;
    ...
    let slope = clamp(length(gradient) / (2.0 * 3.0 / GLYPH_CELL), 0.0, 1.0);
...
fn fs_quad(in: QuadOut) -> @location(0) vec4<f32> {
    ...
    // A cell that holds the shape of a mark rather than a picture of one, to be
    // shaded as a bead of water standing on the surface below it.
    //
    // Nothing else in this greeter asks for a square-cornered quad with a
    // depth: every pane that wants the glass material is rounded — the column,
    // the field, the buttons, the keys — and every quad that samples a drawing
    // or a photograph carries no depth. So the pair is how a caller says which
    // kind of cell it is pointing at, without a field of its own, and
    // `Quad::glyph_material` is the same test on the Rust side so the two
    // cannot drift.
    ...
    if (radius > 0.0) {
        ...
        if (in.shape.y <= 0.0 && (thickness > 0.0 || gloss > 0.0)) {
            ...
            if (slab > 0.0) {
                ...
                let red = behind_at(
                    in.clip.xy + outward * bevel_shift(inset, slab, eta + spread),
                    lod, soften, in.display);
                let green = behind_at(in.clip.xy + outward * shift, lod, soften, in.display);
                let blue = behind_at(
                    in.clip.xy + outward * bevel_shift(inset, slab, eta - spread),
                    lod, soften, in.display);

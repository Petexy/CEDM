...
# The name every package, unit, account and path is built from. Spelt once so
# that the greeter account, the state directory and the configuration
# directory cannot drift apart across four packaging formats.
PACKAGE_NAME="cedm"
GREETER_ACCOUNT="cedm-greeter"
...
# The one place a Rust version is written down is the crate's own
# `rust-version`. Four package definitions each carrying their own copy is four
# chances for the toolchain a package demands and the toolchain the code
# actually compiles under to stop being the same number.
MINIMUM_RUST="$(awk '
    /^\[/ { section = $0; next }
    section == "[package]" \
        && match($0, /^rust-version[[:space:]]*=[[:space:]]*"[^"]+"/) {
        line = substr($0, RSTART, RLENGTH)
        sub(/^rust-version[[:space:]]*=[[:space:]]*"/, "", line)
        sub(/"$/, "", line)
        print line
        exit
    }' "$PROJECT_ROOT/Cargo.toml")"

if [[ ! "$MINIMUM_RUST" =~ ^[0-9]+\.[0-9]+(\.[0-9]+)?$ ]]; then
    echo "could not read [package] rust-version from $PROJECT_ROOT/Cargo.toml" >&2
    exit 1
fi
...
# at a fraction of RAM, and building there means building in memory: a release
# build of this dependency graph writes about 850 MiB, and the `cargo test` that
# makepkg's check() and rpmbuild's %check run adds roughly 4.5 GiB more in the
# dev profile. That is how a package build ends in "No space left on device" —
# or, on a tmpfs mounted with quotas, "Disk quota exceeded" — long after the
# point where anything could be done about it.
#
...
PACKAGE_WORK_ROOT="${CEDM_WORK_DIR:-$PACKAGING_DIR/out/build}"
...
# A relative CEDM_WORK_DIR has to become absolute here rather than at first use:
# the guard in package_work_dir asks whether the work root is inside the
# checkout, and a relative path would answer no to that question however far
# inside it actually is.
if [[ -n "${CEDM_WORK_DIR:-}" ]]; then
    package_set_work_root "$CEDM_WORK_DIR"
fi
...
Point the build somewhere with room using --work-dir DIR (or CEDM_WORK_DIR),

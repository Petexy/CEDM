...
dependency graph needs about 1 GiB to compile plus roughly 4.5 GiB more for the
dev-profile build that check() runs. CEDM_WORK_DIR sets the same thing.

Examples:
  packaging/arch/build.sh
  packaging/arch/build.sh -- --syncdeps
  packaging/arch/build.sh -- --nocheck
  packaging/arch/build.sh --work-dir /var/tmp/cedm
EOF
}
...
work="$(package_work_dir cedm-arch)"
cleanup() {
    if [[ -n "${work:-}" && "$work" == */cedm-arch.* && -d "$work" ]]; then
        rm -rf -- "$work"
    fi
}
trap cleanup EXIT
...
if [[ " ${makepkg_extra[*]} " == *" --nocheck "* ]]; then
    require_free_space "$work" 2048
else
    require_free_space "$work" 8192
fi
...
source_dir="$work/$PACKAGE_NAME-$PACKAGE_VERSION"
source_archive="$work/$PACKAGE_NAME-$PACKAGE_VERSION.tar.gz"
...
# makepkg resolves `install=` beside the PKGBUILD, so the scriptlet has to be
# rendered into the work directory with it.
install -m0644 "$script_dir/cedm.install" "$work/cedm.install"

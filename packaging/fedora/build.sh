...
%check runs this needs several gigabytes. CEDM_WORK_DIR sets the same thing.
EOF
}
...
work="$(package_work_dir cedm-fedora)"
cleanup() {
    if [[ -n "${work:-}" && "$work" == */cedm-fedora.* && -d "$work" ]]; then
        rm -rf -- "$work"
    fi
}
trap cleanup EXIT
...
source_archive="$topdir/SOURCES/$PACKAGE_NAME-$PACKAGE_VERSION.tar.gz"
archive_snapshot "$source_dir" "$source_archive"
install -m0644 "$script_dir/$PACKAGE_NAME.spec" "$topdir/SPECS/$PACKAGE_NAME.spec"
...
rpmbuild "${rpmbuild_args[@]}" "$topdir/SPECS/$PACKAGE_NAME.spec"
